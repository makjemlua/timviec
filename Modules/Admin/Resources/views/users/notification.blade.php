			...
			<span style="display: none">{{ $a=1 }}</span>
				@foreach($notis as $noti)
					<tr>
						...
						<td><!-- Trạng thái -->
							<a href="{{ route('admin.get.action.user.notification',['active', $noti->id]) }}" class="btn {{ $noti->getStatus($noti->no_active)['class'] }}">{{ $noti->getStatus($noti->no_active)['name'] }}</a>
						</td>
						<td><!-- Thao tác -->
							<a class="btn btn-danger" href="{{ route('admin.get.action.user.notification',['delete', $noti->id]) }}" title="Xoá">
