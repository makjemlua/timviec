...
<p>Tham gia các gói <a class="important" href="">Dịch vụ đăng tin</a> để tuyển dụng nhanh và hiệu quả với nhiều quyền lợi hấp dẫn.</p>
...
<table class="table table-striped table-sm">
	...
	<tbody>
		...
		<tr>
			<td>{{ $a++ }}</td>
			<td width="150px">
				<a href="{{ route('employer.get.action.profile',['active', $employer->id]) }}" class="label {{ $employer->getStatus($employer->pr_status)['class'] }}">{{ $employer->getStatus($employer->pr_status)['name'] }}</a>
			</td>
			<td>
				{{ $employer->pr_title }}
			</td>
			<td width="300px">
				<a class="thao-tac xem label label-success" href="{{ route('employer.profile.view', $employer->id) }}" title="Xem hồ sơ">
					<i class="fa fa-eye" aria-hidden="true"></i>Xem
				</a>
				<a class="thao-tac sua label label-primary" href="{{ route('employer.profile.update', $employer->id) }}" title="Cập nhập">
					<i class="fa fa-pencil-square-o" aria-hidden="true"></i>Cập nhập
				</a>
				<a class="thao-tac xoa label label-danger" href="{{ route('employer.get.action.profile', ['delete', $employer->id]) }}" title="Xóa">
